...
class SlidingSheet extends StatefulWidget {
  ...
  /// The [SnapSpec] that defines if and how the sheet should snap.
  final SnapSpec snapSpec;

  /// The base animation duration for the sheet.
  /// Swipes and flings may have a different duration.
  ...
  ///
  /// The parallax effect is an effect that appears when different layers
  /// of backgrounds are moved at different speeds and thereby create
  /// the effect of motion and depth. By moving
  ...
  ///
  /// That means that the user wont be able to close the sheet
  /// using gestures or back button.
  final bool isDismissable;

  /// A callback that gets invoked when a user tried to dismiss the dialog
  /// while [isDimissable] is set to `true`.
  ///
  /// The `backButton` flag indicates whether the user tried to dismiss
  /// the sheet using the backButton, while the `backDrop` flag indicates
  ///  whether the user tried to dismiss the sheet by tapping the backdrop.
  ...
  /// will be scrolled if the content is bigger than the height that
  /// the sheet can expand to.
  ///
  /// The `headerBuilder` and `footerBuilder` can be used to build persistent
  /// widget on top and bottom respectively, that wont be scrolled
  /// but will delegate the interactions on them to the sheet.
  ///
  /// The `listener` callback is being invoked when the sheet gets dragged
  ///  or scrolled with current state information.
  ///
  /// The `snapSpec` can be used to customize the snapping behavior.
  /// There you can custom snap extents, whether the sheet should snap at all,
  /// and how to position those snaps.
  ///
  /// If `addTopViewPaddingOnFullscreen` is set to true,
  /// the sheet will add the status bar height as a top padding to it
  /// in order to avoid the status bar it it is translucent.
  ///
  /// The `cornerRadiusOnFullscreen` parameter can be used to easily
  /// implement the common Material behaviour of sheets to go from rounded
  /// corners to sharp corners when taking up the full screen.
  ///
  /// The `body` parameter can be used to place a widget behind the sheet and
  /// a parallax effect can be applied to it using the `parallaxSpec` parameter.
  ///
  /// The `axisAlignment` parameter can be used to align the sheet on the
  /// horizontal axis when the available width is bigger than the `maxWidth`
  /// of the sheet.
  SlidingSheet({
    Key? key,
  ...
  Widget _buildBody() {
    ...
    return ValueListenableBuilder(
      ...
      builder: (context, dynamic _, body) {
        ...
        assert(
            maxExtent > minExtent,
            'The endExtent must be greater than the min snap extent'
            ' you set on the SnapSpec');
