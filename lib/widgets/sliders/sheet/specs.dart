...
class SnapSpec {
  ...
  /// will animate immidiately to the next valid snap.
  ...
  /// - [SnapPositioning.relativeToSheetHeight] positions the snaps relative
  /// to the total size of the sheet itself. All values must be between 0 and 1.
  /// - [SnapPositioning.pixelOffset] positions the snaps at the given
  /// pixel offset. If the sheet is smaller than the offset,
  /// it will snap to the max possible offset.
  final SnapPositioning positioning;

  /// A callback function that gets called when the [SlidingSheet] snaps.
  final void Function(SheetState, double? snap)? onSnap;

  /// Creates an object that defines if how a [SlidingSheet] should snap.
  const SnapSpec({
    this.snap = true,
  ...
  /// The snap extent that makes header and footer fully visible without
  /// account for vertical padding on the [SlidingSheet].
  static const double headerFooterSnap = -1;

  /// The snap extent that makes the header fully visible without account
  /// for top padding on the [SlidingSheet].
  static const double headerSnap = -2;

  /// The snap extent that makes the footer fully visible without account
  /// for bottom padding on the [SlidingSheet].
  ...
  @override
  String toString() {
    return 'SnapSpec(snap: $snap, snappings: $snappings, initialExtent: '
        '$initialSnap, positioning: $positioning, onSnap: $onSnap)';
  }
...
class ParallaxSpec {
  ...
  /// the minExtent is defined by the lowest snap in the `snappings`
  ///
  /// If endExtent is null, the [SlidingSheet] will use the penultimate extent
  /// on the `snappings` array if there are more than 2 snaps,
  /// otherwise the `maxExtent` will be used.
  ///
  /// **Note that the [SnapPositioning] you set on the [SnapSpec]
  /// will be applied to this extent aswell**
